# MC-Brum-Flow Decomposition & Modernization — Implementation Plan v06
...
| # | Feedback issue (v05 review) | What changed in v06 | Why |
|---|---------------------------|---------------------|-----|
| 1 | Outcome B warming dedupe has a concurrency hole: `acquireSocket()` clears `warmingPromise` to `null` before the warming socket finishes connecting, so a subsequent `warmNextConnection()` call bypasses the guard and starts a second warm connection. | Two fixes: (a) Added `if (warmingSocket && !warmingSocket.destroyed) return;` to the dedupe guard in `warmNextConnection()` so it checks the raw socket ref, not just the promise. (b) Removed the `warmingPromise = null` line from `acquireSocket()` -- the promise is allowed to settle naturally (the `connect`/`error` handlers in `warmNextConnection` already null out both `warmingSocket` and `warmingPromise`). Added a test case for the interleaving scenario (acquireSocket called while warming is in-flight, followed by a concurrent `warmNextConnection` call). | Without fix (a), the `warmingSocket` ref could be non-null while `warmingPromise` is already nulled by `acquireSocket`, letting a new warming attempt slip through. Without fix (b), nulling the promise in `acquireSocket` is the root cause -- the caller takes ownership of the promise's result, but the underlying socket is still connecting; the connect/error handler is the only safe place to clear both refs. |
| 2 | Changes vs v04 table row #2 says `createMultiChunkClient` but the actual helper is `createMultiChunkServer`. | Fixed the name to `createMultiChunkServer` in the Changes vs v04 summary. | Typo -- the helper creates a mock TCP **server**, not a client. |

## Changes vs v04

| # | Feedback issue (v04 review) | What changed in v05 | Why |
|---|---------------------------|---------------------|-----|
| 1 | Outcome B `warmingPromise` never settles on error -- `acquireSocket` hangs forever if the warming socket errors before `connect`. Shutdown via `drainPool()` does not destroy an in-flight warming socket. | Replaced the `warmingPromise: Promise<net.Socket> | null` pattern with a tracked `warmingSocket: net.Socket | null` reference. The warming promise now resolves `null` on error instead of silently swallowing. `acquireSocket` checks for `null` resolution and falls back to a new socket. `drainPool()` now destroys both `warmSocket` and `warmingSocket`. | A promise that never settles is a hang bug. If the warming socket errors before connecting, any caller awaiting `warmingPromise` would block forever. By resolving `null` on error and tracking the raw socket separately, shutdown can always destroy it and callers always get a definitive result. |
| 2 | Mock TCP server in Task 4.3 uses `JSON.parse(chunk.toString().trim())` which breaks on partial chunks or multiple newline-framed messages in a single chunk -- does not match the newline-delimited protocol. | Replaced the mock server's `on('data')` handler with a per-connection buffer that splits on `\n` in a `while` loop. Added `createMultiChunkServer` and `createTrailingByteServer` test helpers for split-delivery and trailing-byte scenarios. | The real protocol is newline-framed. A mock that assumes one-JSON-per-chunk would pass tests that the production code would fail, defeating the purpose of protocol-level testing. The buffer-and-split pattern matches how the production `onData` handler works. |
| 3 | Task 4.2 verification says "both Outcome A and Outcome B test suites pass" but Task 4.3 says "only the chosen outcome's tests need to pass in CI." Contradictory. | Standardized on `describe.skip` for the unchosen outcome suite. Task 4.2 and 4.3 verification now both say: "chosen outcome suite + shared tests pass; unchosen outcome suite is wrapped in `describe.skip`." | Both suites cannot pass simultaneously in CI because only one outcome's implementation is shipped. `describe.skip` keeps the unchosen tests in the codebase for future reference without causing CI failures. |
...
let warmingSocket: net.Socket | null = null;  // raw ref to in-flight warming socket (for shutdown)
let warmingPromise: Promise<net.Socket | null> | null = null;
...
function warmNextConnection(host: string, port: number): void {
  ...
  if (warmingSocket && !warmingSocket.destroyed) return;  // socket is connecting (ref survives even if promise was consumed by acquireSocket)
  if (warmingPromise) return;  // connection attempt already in flight (promise not yet consumed)

  const socket = net.createConnection({ host, port });
  warmingSocket = socket;  // track raw ref so drainPool() can destroy it

  warmingPromise = new Promise<net.Socket | null>((resolve) => {
    socket.once('connect', () => {
      warmSocket = socket;
      warmingSocket = null;
      warmingPromise = null;
    ...
    socket.once('error', () => {
      ...
      warmingPromise = null;
      socket.destroy();
      resolve(null);  // resolve null, never hang
    });
  });
}
...
function acquireSocket(host: string, port: number): Promise<net.Socket> {
  ...
  // Do NOT null out warmingPromise here -- let the connect/error handler
  // in warmNextConnection clear both warmingSocket and warmingPromise
  // when the socket finishes connecting or errors. Nulling it here would
  // let a concurrent warmNextConnection() call bypass the dedupe guard.
  if (warmingPromise) {
    const promise = warmingPromise;
    return promise.then((socket) => {
      warmSocket = null;
      if (socket && !socket.destroyed) return socket;
      // Warming failed -- fall back to a new socket
      return createFreshSocket(host, port);
    });
  }
...
function createFreshSocket(host: string, port: number): Promise<net.Socket> {
  return new Promise((resolve, reject) => {
    const socket = net.createConnection({ host, port });
...
export function drainPool(): void {
  ...
  warmSocket = null;
  // Destroy the in-flight warming socket if it has not connected yet
  if (warmingSocket && !warmingSocket.destroyed) warmingSocket.destroy();
  warmingSocket = null;
  warmingPromise = null;
}
```
...
**Verification:**
- `npm test` passes -- chosen outcome suite + shared `sendMaxMcpCommand` tests pass; unchosen outcome suite is wrapped in `describe.skip`
- Manual test: rapid push-to-max calls reuse connections or use warm sockets (check log output)
- Server shutdown cleanly drains pool and exits (including in-flight warming sockets in Outcome B)
...
| Warming dedupe survives acquireSocket consuming the promise (interleaving) | N/A | Yes |
| Multi-chunk response assembled correctly | Yes | Yes |
| Socket timeout triggers rejection | Yes | Yes |

```typescript
// Mock TCP server helper for tests -- uses newline-framed buffering
  ...
  return net.createServer((socket) => {
    ...
    socket.on('data', (chunk) => {
      ...
      while ((newlineIndex = buffer.indexOf('\n')) !== -1) {
        ...
        if (!frame) continue;  // skip empty lines
        ...
        if (!options.keepAlive) {
          ...
          return;  // stop processing buffer -- socket is closing
        }
      }
    });
  });
...
// Helper: creates a mock server that sends responses split across multiple chunks
// (e.g., first 10 bytes, then the rest after a short delay)
function createMultiChunkServer(splitAt: number): net.Server {
  return net.createServer((socket) => {
    let buffer = '';
    socket.on('data', (chunk) => {
      buffer += chunk.toString('utf8');
      ...
      while ((newlineIndex = buffer.indexOf('\n')) !== -1) {
        ...
        // Send first part immediately, rest after a tick
        socket.write(fullResponse.slice(0, splitAt));
        setTimeout(() => {
          if (!socket.destroyed) socket.write(fullResponse.slice(splitAt));
        }, 5);
      }
    });
  });
...
// Helper: creates a mock server that appends trailing bytes after the \n
// (simulates protocol violation / unexpected extra data)
function createTrailingByteServer(): net.Server {
  return net.createServer((socket) => {
    let buffer = '';
    socket.on('data', (chunk) => {
      buffer += chunk.toString('utf8');
      ...
      while ((newlineIndex = buffer.indexOf('\n')) !== -1) {
        ...
        // Write response + newline + trailing garbage bytes
        socket.write(response + '\n' + 'TRAILING');
      }
    });
  });
...
// Use describe.skip for the unchosen outcome so tests stay in codebase
// but do not fail CI. Swap skip/active based on decision gate result.

describe('connection pooling (Outcome A)', () => {  // or describe.skip if Outcome B is chosen
...
describe('connection warming (Outcome B)', () => {  // or describe.skip if Outcome A is chosen
  let mockServer: net.Server;
  let serverPort: number;
  ...
  it('falls back to new socket when warming promise resolved null (error)', async () => { /* ... */ });
  it('starts warming next connection after response', async () => { /* ... */ });
  it('deduplicates concurrent warming attempts', async () => { /* ... */ });
  it('deduplicates warming when acquireSocket consumed the promise but socket is still connecting', async () => {
    // Interleaving scenario:
    // 1. warmNextConnection() starts -- sets warmingSocket + warmingPromise
    // 2. acquireSocket() takes warmingPromise (but does NOT null it -- fix in v06)
    // 3. Before the warming socket finishes connecting, warmNextConnection() is called again
    // 4. The warmingSocket ref is still non-null and not destroyed, so the guard returns early
    // 5. No second warming socket is created
    /* ... */
  });
  it('drainPool destroys warm socket and in-flight warming socket', async () => { /* ... */ });
});

// Shared tests (both outcomes -- always active)
describe('sendMaxMcpCommand', () => {
  it('assembles multi-chunk response', async () => { /* uses createMultiChunkServer */ });
  it('rejects on socket timeout', async () => { /* ... */ });
  it('rejects on socket error', async () => { /* ... */ });
});
```

**Note:** The unchosen outcome's `describe` block must be changed to `describe.skip(...)` after the decision gate. This keeps the tests in the codebase for future reference without causing CI failures. The chosen outcome's suite + the shared `sendMaxMcpCommand` suite must both pass.
...
3. Create `max-mcp-client.test.ts` -- spin up mock TCP servers using the newline-buffered helpers (`createMockTcpServer`, `createMultiChunkServer`, `createTrailingByteServer`), test pool/warming behavior against the appropriate mock
4. Test cleanup: ensure `drainPool()` is called in `afterEach` for pool/warming tests
5. After decision gate: wrap the unchosen outcome's `describe(...)` in `describe.skip(...)`

**Verification:**
- `npm test` -- chosen outcome suite + shared `sendMaxMcpCommand` tests pass; unchosen outcome suite is `describe.skip`
