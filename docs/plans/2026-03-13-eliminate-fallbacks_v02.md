# Eliminate Unnecessary Fallbacks — Implementation Plan v02
...
- [ ] Task 3: Surface `resolvePaths` failures via toast and `pathResolutionError` flag
...
- [ ] Task 22: Add success and error toasts to `pushToMax` / `submitRender` in OutputPreviewPanel
...
However, delete routes (`DELETE /scenes/:id`, `DELETE /node-configs/:id`, `DELETE /cameras/:id`) return `{ success: true }` without a `data` field. The current line 58 (`if (data.data === undefined) throw ...`) would break these. Delete calls are typed as `request<void>`, so we need to handle this case.

**Files:**
- `client/src/lib/api.ts` (lines 56-59) -- tighten the success check while supporting `{ success: true }` without `data`
- `client/src/lib/api.test.ts` -- add tests for edge cases

**Steps:**
1. Change lines 56-59 from:
   ```ts
   ...
   return data.data as T;
   ```
   This catches both `success: false` and missing/undefined `success`. For delete routes returning `{ success: true }` without `data`, `data.data` is `undefined`, which is returned as `T` (where `T = void`). This is correct -- the caller declared `request<void>` and doesn't use the return value.

2. Add tests:
   - Response with `{ success: undefined }` throws
   - Response with `{ data: 'foo' }` (no success field) throws
   - Response with `{ success: true, data: 'foo' }` succeeds and returns `'foo'`
   - Response with `{ success: true }` (no data field) succeeds and returns `undefined`

**Verification:**
- `npm test` passes
- Existing delete operations (`deleteScene`, `deleteNodeConfig`, `deleteCamera`) continue to work -- they are typed as `request<void>` and the callers do not use the return value

---

### Task 3: Surface `resolvePaths` failures via toast and `pathResolutionError` flag

`flowStore.ts` line 667-677: when `resolvePaths` throws, the catch block does `console.warn` and silently sets `resolvedPaths: [], pathCount: 0`. The user sees all their output paths vanish with no explanation. Additionally, push/submit buttons should be disabled when path resolution is stale.

**Files:**
- `client/src/stores/flowStore.ts` (lines 667-677) -- add toast on failure, keep paths stale, add `pathResolutionError` flag
...
3. In `OutputPreviewPanel.tsx`, subscribe to `pathResolutionError`:
   ```ts
...
4. Add a test that verifies the store sets `pathResolutionError: true` on resolution failure and does NOT clear `resolvedPaths` (mock `api.resolvePaths` to reject).
...
The type guard should check the minimum required fields that the store actually depends on for downstream logic: `id` (string), `name` (string), and `max_handle` (number). The remaining fields (`scene_id`, `max_class`, `created_at`, `updated_at`) are present when the server returns valid DB rows but are not critical for the store's merge/node-creation logic.
...
1. Add a validation function near the top of `flowStore.ts`:
   ...
   ```
   Note: We check the three fields the store's merge and node-creation logic actually reads. The `Camera` type requires more fields (scene_id, max_class, created_at, updated_at) but those come from the DB response and are always present in practice. The guard catches malformed data without being so strict it rejects valid DB rows with extra/unexpected fields.
...
**Files:**
- `server/src/services/flowResolver.ts` (lines 206-208) -- add a `warnings` array to the return type and populate it
- `server/src/services/flowResolver.test.ts` -- update existing test to verify warning

**Steps:**
1. Add `warnings` to `ResolvedFlowPath`:
   ```ts
...
4. **Consume warnings in the server**: In `syncSceneToMaxNow` (max-sync.ts), after resolving paths and selecting a target, emit any warnings via `emitMaxLog`:
   ```ts
...
5. Update the existing test "uses camera node label as fallback when camera_id is not in cameras map" to also check `result[0].warnings` contains the expected warning string.

6. Make `warnings` optional (`warnings?: string[]`) in the client's `ResolvedPath` interface in `flowStore.ts` so the client gracefully handles its absence during the deploy gap.

**Verification:**
- `npm test` passes (update existing test expectations)
- Warnings appear in the Max Debug Panel when a camera DB record is missing
...
**Verification:**
- `npm test` passes
- The warning string is included in the path's `warnings` array and surfaced via `emitMaxLog` (from Task 8's consumer code)
...
We should import and use it rather than defining a duplicate interface.

**Files:**
- `client/src/stores/flowStore.ts` (line 770)

**Steps:**
1. Add `FlowConfig` to the existing import from `@shared/types` on line 6:
   ```ts
   import type { Scene, Camera, StudioDefault, NodeConfig, FlowNode, FlowEdge, FlowConfig, NodeType, MaxSyncState } from '@shared/types';
   ```

2. Replace `(row: any)` with `(row: FlowConfig)` on line 770.

3. Add a runtime guard at the top of the handler to protect against malformed socket payloads:
   ```ts
...
**IMPORTANT:** The store's `pushToMax` and `submitRender` do NOT call `showToast()`. They only set `error` state and add sync log entries. The OutputPreviewPanel must handle both success and failure feedback.

Looking at the actual store code:
- `pushToMax` (line 937-1003): returns `true` on success, `false` on failure. Sets `error` state but does NOT toast.
- `submitRender` (line 1006-1024): returns `true` on success, `false` on failure. Sets `error` state but does NOT toast.

**Files:**
- `client/src/components/output/OutputPreviewPanel.tsx` (lines 23-29, 32-40)

**Steps:**
1. Add `showToast` to the component's store selectors:
   ```ts
   const showToast = useFlowStore((s) => s.showToast);
   ```

2. Update `handlePushToMax`:
   ```ts
   const handlePushToMax = async (pathKey: string) => {
     ...
     try {
       ...
       if (success) {
         ...
         showToast('Push to 3ds Max failed', 'error');
       }
     } finally {
       setPushing(false);
     }
...
3. Update `handleSubmitRender`:
   ...
   Note: The store's `pushToMax` also handles the `max_camera_not_found` case by showing a `cameraMatchPrompt` and setting `error` state. The panel's error toast for that case will say "Push to 3ds Max failed", which is appropriate -- the detailed error is visible in the camera match prompt and the sync log.

**Verification:**
- `npm test` passes
- User sees a green toast on successful push/submit
- User sees a red toast on failure
...
Adding `warnings: string[]` to `ResolvedFlowPath` in Task 8 touches `server/src/services/flowResolver.ts`. The client's `ResolvedPath` interface in `flowStore.ts` is a separate type — make `warnings` optional there (`warnings?: string[]`) so the client gracefully handles its absence during the deploy gap. The server-side `ResolvedFlowPath` type should have `warnings: string[]` (non-optional) since it's always populated.
...
The store's `pushToMax` and `submitRender` do NOT call `showToast()`. They set `error` state and add sync log entries. OutputPreviewPanel is responsible for showing both success and failure toasts based on the boolean return value. This is different from Tasks 13-14 where the store itself toasts — the pattern difference exists because `pushToMax`/`submitRender` have complex error handling (camera match prompts, sync logs) that the panel shouldn't duplicate.
